...
__device__ void depositPheromones_dev_transposed(const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon_transposed, double* __restrict__ kol_enter_transposed)
{
    ...
    // Каждый warp обрабатывает один value_idx (0..4) и несколько параметров
    int warp_id = (bx * blockDim.x + tx) / WARP_SIZE;
    int lane_id = tx % WARP_SIZE;
    int value_idx = warp_id % MAX_VALUE_SIZE;  // Каждому value_idx несколько warp'ов

    if (value_idx >= MAX_VALUE_SIZE) return;

    // Количество warp'ов на value_idx
    int total_warps = (gridDim.x * blockDim.x + WARP_SIZE - 1) / WARP_SIZE;
    int warps_per_value = (total_warps + MAX_VALUE_SIZE - 1) / MAX_VALUE_SIZE;
    int warp_in_value = warp_id / MAX_VALUE_SIZE;

    // Распределяем параметры между warp'ами одного value_idx
    int params_per_warp = (PARAMETR_SIZE + warps_per_value - 1) / warps_per_value;
    int start_param = warp_in_value * params_per_warp;
    int end_param = min(start_param + params_per_warp, PARAMETR_SIZE);

    // Распределяем работу внутри warp'а
    int params_per_lane = (params_per_warp + WARP_SIZE - 1) / WARP_SIZE;
    int param_offset = lane_id * params_per_lane;

    for (int p = 0; p < params_per_lane; p++) {
        int param_idx = start_param + param_offset + p;
        if (param_idx >= end_param) break;
        ...
        // Можно оптимизировать: обрабатывать агентов блоками
        for (int ant_id = 0; ant_id < ANT_SIZE; ant_id++) {
            ...
            if (agent_node[ant_id * PARAMETR_SIZE + param_idx] == value_idx) {
                ...
                // Ваша оригинальная логика вычисления феромона
#if OPTIMIZE_MIN_1
                const double delta = MAX_PARAMETR_VALUE_TO_MIN_OPT - OF[ant_id];
...
__device__ void antColonyOptimization_dev(double* __restrict__ dev_parametr_value, double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {
...
__device__ void antColonyOptimization_dev_transposed(double* __restrict__ dev_parametr_value_transposed, double* __restrict__ norm_matrix_probability_transposed, int* __restrict__ agent_node, double* __restrict__ OF, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {
...
__global__ void antColonyOptimization(const bool go_transposed, double* __restrict__ dev_parametr_value, double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF,  double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {  // Pre-allocated buffer
    if (go_transposed) {
        antColonyOptimization_dev_transposed(dev_parametr_value, norm_matrix_probability, agent_node, OF, maxOf_dev, minOf_dev, kol_hash_fail, global_params_buffer);
    }
    else {
        antColonyOptimization_dev(dev_parametr_value, norm_matrix_probability, agent_node, OF, maxOf_dev, minOf_dev, kol_hash_fail, global_params_buffer);
    }
}
...
// ==================== ПАРАЛЛЕЛЬНОЕ ЯДРО ====================
__global__ void parallelACOIteration(const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon, double* __restrict__ kol_enter, double* __restrict__ norm_matrix_probability, double* __restrict__ dev_parametr_value, double* __restrict__ norm_matrix_probability_ant, int* __restrict__ agent_node_ant, double* __restrict__ OF_ant,  double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ dev_agent_params) {
...
PerformanceMetrics run_aco_iterations_4function(const bool go_min_parametrs, const bool go_transposed, int num_iterations) {
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        antColonyOptimization << <ant_blocks, threads_per_block, 0, compute_stream >> > (go_transposed,dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF, dev_max, dev_min, dev_hash_fail, dev_agent_params);
        CUDA_CHECK(cudaEventRecord(stop, compute_stream));
        CUDA_CHECK(cudaStreamSynchronize(compute_stream));
...
double calculate_max_parametr_size_in_gpu_memory() {
    ...
    for (int device = 0; device < deviceCount; ++device) {
        ...
        GlobalMem = GlobalMem - sizeof(double) * 2 - sizeof(int) - ANT_SIZE * sizeof(double);
...
int print_information() {
    ...
    for (int device = 0; device < deviceCount; ++device) {
        ...
        std::cout << "GPU Global Memory Hash Table: " <<  " MB" << std::endl;
        logFile << "GPU Global Memory Hash Table: " <<  " MB ";
