...
// Фаза 2: Перемещение муравьев-агентов
#if (PARAMETR_SIZE < MAX_SHARED)
__global__ void antColonyOptimization(
#if (MAX_VALUE_SIZE * PARAMETR_SIZE > MAX_CONST)
    double* __restrict__ dev_parametr_value,
#endif
    double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF, 
#if (GO_HASH)
    HashEntry* __restrict__ hashTable, 
#endif
    double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail) {
...
#pragma unroll
    for (int param_idx = tx; param_idx < PARAMETR_SIZE; param_idx += blockDim.x) {
        ...
        int k = 0;
#if (NON_WHILE_ANT)
        if (randomValue < norm_matrix_probability[param_idx * MAX_VALUE_SIZE]) { k = 0; }
        else if (randomValue < norm_matrix_probability[param_idx * MAX_VALUE_SIZE] + 1) { k = 1; }
        else if (randomValue < norm_matrix_probability[param_idx * MAX_VALUE_SIZE] + 2) { k = 2; }
        else if (randomValue < norm_matrix_probability[param_idx * MAX_VALUE_SIZE] + 3) { k = 3; }
        else { k = 4; }
#else
...
__global__ void antColonyOptimization( double* __restrict__ dev_parametr_value, double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF, 
#if (GO_HASH)
    HashEntry* __restrict__ hashTable, 
#endif
    double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {  // Pre-allocated buffer
...
// Фаза 3: Испарение феромонов
__global__ void evaporatePheromones(double* __restrict__ pheromon) {
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;

    // Обрабатываем несколько элементов на поток для лучшей загрузки
    for (int idx = tid; idx < PARAMETR_SIZE * MAX_VALUE_SIZE; idx += stride) {
        pheromon[idx] *= PARAMETR_RO;
    }
}

// Фаза 4: Добавление феромонов
__global__ void depositPheromones(const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon, double* __restrict__ kol_enter) {
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    int param_idx = bx * blockDim.x + tx;
    ...
    // Оптимальный unroll фактор зависит от ANT_SIZE и архитектуры GPU
    // Для современных GPU 4-8 обычно оптимально
#pragma unroll 4
    for (int ant_id = 0; ant_id < ANT_SIZE; ant_id++) {
        if (OF[ant_id] != ZERO_HASH_RESULT) {
            int k = agent_node[ant_id * PARAMETR_SIZE + param_idx];
            ...
            double delta = MAX_PARAMETR_VALUE_TO_MIN_OPT - OF[ant_id];
            if (delta > 0) {
                atomicAdd(&pheromon[idx], PARAMETR_Q * delta);
            }
#elif (OPTIMIZE_MIN_2)
            double of_val = (OF[ant_id] == 0) ? 1e-7 : OF[ant_id];
            atomicAdd(&pheromon[idx], PARAMETR_Q / of_val);
...
__global__ void parallelACOIteration(const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon, double* __restrict__ kol_enter, double* __restrict__ norm_matrix_probability,
    ...
    double* __restrict__ dev_parametr_value,
#endif
    double* __restrict__ norm_matrix_probability_ant, int* __restrict__ agent_node_ant, double* __restrict__ OF_ant, 
#if (GO_HASH)
    HashEntry* __restrict__ hashTable, 
#endif
    double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail
...
/*
// ==================== ОБЪЕДИНЕННОЕ ЯДРО С ЦИКЛОМ ИТЕРАЦИЙ ====================
// Фаза 1+2+3+4: Метод муравьиных колоний в одном вызове
// USE_SHARED_MEMORY - флаг использования shared memory (true/false)
// PARAMS_PER_BLOCK - количество параметров, обрабатываемых одним блоком
template<bool USE_SHARED_MEMORY, int PARAMS_PER_BLOCK>
__global__ void ComputePheromonesAdaptive(
    double* __restrict__ global_pheromon,           // Феромоны для каждого значения параметра
    double* __restrict__ global_kol_enter,          // Количество посещений каждого значения
    double* __restrict__ global_norm_matrix_probability, // Нормализованные вероятности
    int* __restrict__ global_agent_node,            // Выбранные узлы для агентов
    double* __restrict__ global_OF,                 // Значения фитнес-функции для агентов
#if (MAX_VALUE_SIZE * PARAMETR_SIZE > MAX_CONST)
    double* __restrict__ dev_parametr_value,        // Значения параметров
#endif
#if (GO_HASH)
    HashEntry* __restrict__ hashTable,              // Хэш-таблица для кэширования результатов
#endif
    double* __restrict__ maxOf_dev,                 // Максимальное значение фитнес-функции
    double* __restrict__ minOf_dev,                 // Минимальное значение фитнес-функции
    int* __restrict__ kol_hash_fail,               // Счетчик попаданий в кэш
    int num_iterations) {                          // Количество итераций алгоритма

    int tx = threadIdx.x;              // Индекс потока в блоке
    int bx = blockIdx.x;               // Индекс блока
    int param_idx = bx * blockDim.x + tx; // Глобальный индекс параметра

    // ==================== ВЫДЕЛЕНИЕ SHARED MEMORY ====================
    // Используется условное выделение через шаблонный параметр
    __shared__ double s_pheromon[USE_SHARED_MEMORY ? PARAMS_PER_BLOCK * MAX_VALUE_SIZE : 1];
    __shared__ double s_kol_enter[USE_SHARED_MEMORY ? PARAMS_PER_BLOCK * MAX_VALUE_SIZE : 1];
    __shared__ double s_agent_params[USE_SHARED_MEMORY ? PARAMETR_SIZE : 1];
    __shared__ int s_agent_node[USE_SHARED_MEMORY ? PARAMETR_SIZE : 1];
    __shared__ curandState s_curand_states[BLOCK_SIZE];     // Генераторы случайных чисел для всех потоков

    // ==================== ИНИЦИАЛИЗАЦИЯ ГЕНЕРАТОРА СЛУЧАЙНЫХ ЧИСЕЛ ====================
    // Инициализируем отдельный генератор для каждого потока
    curand_init(clock64() + param_idx, 0, 0, &s_curand_states[tx]);
    __syncthreads();

    // ==================== ЗАГРУЗКА ДАННЫХ В SHARED MEMORY ====================
    if (USE_SHARED_MEMORY) {
        // Вычисляем, какие параметры обрабатывает этот блок
        int block_start_param = bx * PARAMS_PER_BLOCK;
        int local_param_idx = tx;  // Локальный индекс внутри блока

        if (local_param_idx < PARAMS_PER_BLOCK &&
            (block_start_param + local_param_idx) < PARAMETR_SIZE) {

            int global_param_idx = block_start_param + local_param_idx;

            // Загружаем феромоны и счетчики посещений для всех значений этого параметра
            for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                int global_idx = global_param_idx * MAX_VALUE_SIZE + i;
                int local_idx = local_param_idx * MAX_VALUE_SIZE + i;

                if (global_idx < PARAMETR_SIZE * MAX_VALUE_SIZE &&
                    local_idx < PARAMS_PER_BLOCK * MAX_VALUE_SIZE) {
                    s_pheromon[local_idx] = global_pheromon[global_idx];
                    s_kol_enter[local_idx] = global_kol_enter[global_idx];
                }
            }
        }
        __syncthreads();
    ...
    for (int iteration = 0; iteration < num_iterations; iteration++) {
        ...
        // ФАЗА 1: ВЫЧИСЛЕНИЕ ВЕРОЯТНОСТЕЙ ВЫБОРА ЗНАЧЕНИЙ ПАРАМЕТРОВ
        if (param_idx < PARAMETR_SIZE) {
            double sum = 0.0;

            // Вычисляем сумму феромонов для данного параметра
            for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                int idx = param_idx * MAX_VALUE_SIZE + i;
                if (idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                    if (USE_SHARED_MEMORY) {
                        // Определяем локальный индекс в shared memory
                        int block_start_param = bx * PARAMS_PER_BLOCK;
                        int local_param_idx = param_idx - block_start_param;
                        if (local_param_idx >= 0 && local_param_idx < PARAMS_PER_BLOCK) {
                            int local_idx = local_param_idx * MAX_VALUE_SIZE + i;
                            if (local_idx < PARAMS_PER_BLOCK * MAX_VALUE_SIZE) {
                                sum += s_pheromon[local_idx];
                            }
                        }
                    }
                    else {
                        sum += global_pheromon[idx];
                    }
                }
            }

            double inv_sum = (sum > 1e-10) ? 1.0 / sum : 1.0;
            double prob_sum = 0.0;

            // Первый проход: вычисляем ненормализованные вероятности
            double probabilities[MAX_VALUE_SIZE];
            for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                int idx = param_idx * MAX_VALUE_SIZE + i;
                double pheromon_val = 0.0;
                double kol_val = 0.0;
                ...
                if (idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                    if (USE_SHARED_MEMORY) {
                        ...
                        if (local_param_idx >= 0 && local_param_idx < PARAMS_PER_BLOCK) {
                            int local_idx = local_param_idx * MAX_VALUE_SIZE + i;
                            if (local_idx < PARAMS_PER_BLOCK * MAX_VALUE_SIZE) {
                                pheromon_val = s_pheromon[local_idx];
                                kol_val = s_kol_enter[local_idx];
                            }
                        }
                    }
                    else {
                        pheromon_val = global_pheromon[idx];
                        kol_val = global_kol_enter[idx];
                    }

                    double pheromon_norm = pheromon_val * inv_sum;
                    double prob = 0.0;

                    // Формула вероятности: обратное к количеству посещений + нормированный феромон
                    if (kol_val > 1e-10 && pheromon_norm > 1e-10) {
                        prob = 1.0 / kol_val + pheromon_norm;
                    }
            ...
            // Нормализация и сохранение кумулятивных вероятностей
            double cumulative = 0.0;
            double inv_prob_sum = (prob_sum > 1e-10) ? 1.0 / prob_sum : 1.0;

            for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                int idx = param_idx * MAX_VALUE_SIZE + i;
                if (idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                    cumulative += probabilities[i] * inv_prob_sum;
                    global_norm_matrix_probability[idx] = cumulative;
                }
            }

            // Гарантируем, что последнее значение равно 1.0
            if (tx == 0) {
                int last_idx = param_idx * MAX_VALUE_SIZE + MAX_VALUE_SIZE - 1;
                if (last_idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                    global_norm_matrix_probability[last_idx] = 1.0;
                }
            }
        }
        __syncthreads();

        // ФАЗА 2: СОЗДАНИЕ АГЕНТОВ (ВЫБОР ЗНАЧЕНИЙ ПАРАМЕТРОВ)
        if (USE_SHARED_MEMORY) {
            // Каждый поток выбирает значение для своего параметра
            if (tx < PARAMETR_SIZE && bx == 0) { // Только первый блок создает агентов
                double randomValue = curand_uniform(&s_curand_states[tx]);

                // Бинарный поиск выбранного значения по кумулятивным вероятностям
                int selected_index = 0;
                int left = 0, right = MAX_VALUE_SIZE - 1;

                while (left <= right) {
                    int mid = (left + right) / 2;
                    double prob = global_norm_matrix_probability[tx * MAX_VALUE_SIZE + mid];
                ...
                // Сохраняем выбранные узлы и значения параметров
                s_agent_node[tx] = selected_index;
#if (MAX_VALUE_SIZE * PARAMETR_SIZE < MAX_CONST)
                s_agent_params[tx] = dev_parametr_value[tx * MAX_VALUE_SIZE + selected_index];
#else
                s_agent_params[tx] = dev_parametr_value[tx * MAX_VALUE_SIZE + selected_index];
#endif
            }
            ...
            // Реализация без shared memory
            if (param_idx < PARAMETR_SIZE && bx < ANT_SIZE) {
                int ant_id = bx;
                double randomValue = curand_uniform(&s_curand_states[tx]);
                ...
                for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                    int idx = param_idx * MAX_VALUE_SIZE + i;
                    if (randomValue <= global_norm_matrix_probability[idx]) {
                        selected_index = i;
                        break;
                    }
                }

                // Сохраняем в global memory
                int agent_idx = ant_id * PARAMETR_SIZE + param_idx;
                if (agent_idx < ANT_SIZE * PARAMETR_SIZE) {
                    global_agent_node[agent_idx] = selected_index;
                }
            }
        }
        __syncthreads();

        // ФАЗА 3: ВЫЧИСЛЕНИЕ ФИТНЕС-ФУНКЦИИ
        if (tx == 0 && bx < ANT_SIZE) {
            int ant_id = bx;
            double fitness;

            if (USE_SHARED_MEMORY && bx == 0) {
                // Используем shared memory для первого агента
#if (GO_HASH)
                // Проверяем кэш
                double cached = getCachedResult(hashTable, s_agent_node, 0);
                if (cached < 0.0) {
                    // Вычисляем функцию
                    fitness = BenchShafferaFunction(s_agent_params);
                    saveToCache(hashTable, s_agent_node, 0, fitness);
                }
                else {
                    fitness = cached;
                    atomicAdd(kol_hash_fail, 1);
                ...
                // Чтение из global memory для остальных агентов
                double agent_params[PARAMETR_SIZE];

                for (int i = 0; i < PARAMETR_SIZE; i++) {
                    int agent_idx = ant_id * PARAMETR_SIZE + i;
                    if (agent_idx < ANT_SIZE * PARAMETR_SIZE) {
                        int node = global_agent_node[agent_idx];
                        int param_val_idx = i * MAX_VALUE_SIZE + node;
                        if (param_val_idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                            agent_params[i] = dev_parametr_value[param_val_idx];
                        }
                    }
                }

#if (GO_HASH)
                double cached = getCachedResult(hashTable, global_agent_node + ant_id * PARAMETR_SIZE, ant_id);
                if (cached < 0.0) {
                    fitness = BenchShafferaFunction(agent_params);
                    saveToCache(hashTable, global_agent_node + ant_id * PARAMETR_SIZE, ant_id, fitness);
                }
                else {
                    fitness = cached;
                    atomicAdd(kol_hash_fail, 1);
            ...
            if (USE_SHARED_MEMORY) {
                ...
                if (local_param_idx >= 0 && local_param_idx < PARAMS_PER_BLOCK) {
                    ...
                    for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                        int local_idx = local_param_idx * MAX_VALUE_SIZE + i;
                        if (local_idx < PARAMS_PER_BLOCK * MAX_VALUE_SIZE) {
                            s_pheromon[local_idx] *= PARAMETR_RO;
                        }
                    }

                    // Добавление нового феромона от лучших агентов
                    if (bx < ANT_SIZE) {
                        int ant_id = bx;
                        if (global_OF[ant_id] != ZERO_HASH_RESULT) {
                            int selected_node = s_agent_node[local_param_idx];
                            int idx = local_param_idx * MAX_VALUE_SIZE + selected_node;

                            if (idx < PARAMS_PER_BLOCK * MAX_VALUE_SIZE) {
                                s_kol_enter[idx] += 1.0;

                                // Разные стратегии обновления феромонов
#if (OPTIMIZE_MIN_1)
                                double delta = MAX_PARAMETR_VALUE_TO_MIN_OPT - global_OF[ant_id];
                                if (delta > 0) s_pheromon[idx] += PARAMETR_Q * delta;
#elif (OPTIMIZE_MIN_2)
                                double of_val = (fabs(global_OF[ant_id]) < 1e-10) ? 1e-7 : global_OF[ant_id];
                                s_pheromon[idx] += PARAMETR_Q / of_val;
#elif (OPTIMIZE_MAX)
                                s_pheromon[idx] += PARAMETR_Q * global_OF[ant_id];
#endif
                            }
                        }
                    }
                ...
                // Обновление напрямую в global memory
                for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                    int idx = param_idx * MAX_VALUE_SIZE + i;
                    if (idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                        global_pheromon[idx] *= PARAMETR_RO;
                    }
                }
                ...
                for (int ant_id = 0; ant_id < ANT_SIZE; ant_id++) {
                    if (global_OF[ant_id] != ZERO_HASH_RESULT) {
                        int agent_idx = ant_id * PARAMETR_SIZE + param_idx;
                        if (agent_idx < ANT_SIZE * PARAMETR_SIZE) {
                            int selected_node = global_agent_node[agent_idx];
                            int idx = param_idx * MAX_VALUE_SIZE + selected_node;

                            if (idx < PARAMETR_SIZE * MAX_VALUE_SIZE) {
                                atomicAdd(&global_kol_enter[idx], 1.0);

#if (OPTIMIZE_MIN_1)
                                double delta = MAX_PARAMETR_VALUE_TO_MIN_OPT - global_OF[ant_id];
                                if (delta > 0) atomicAdd(&global_pheromon[idx], PARAMETR_Q * delta);
#elif (OPTIMIZE_MIN_2)
                                double of_val = (fabs(global_OF[ant_id]) < 1e-10) ? 1e-7 : global_OF[ant_id];
                                atomicAdd(&global_pheromon[idx], PARAMETR_Q / of_val);
#elif (OPTIMIZE_MAX)
                                atomicAdd(&global_pheromon[idx], PARAMETR_Q * global_OF[ant_id]);
#endif
                            }
                        }
                    }
    ...
    // ==================== СОХРАНЕНИЕ РЕЗУЛЬТАТОВ ИЗ SHARED MEMORY ====================
    if (USE_SHARED_MEMORY && param_idx < PARAMETR_SIZE) {
        int block_start_param = bx * PARAMS_PER_BLOCK;
        int local_param_idx = param_idx - block_start_param;

        if (local_param_idx >= 0 && local_param_idx < PARAMS_PER_BLOCK) {
            for (int i = 0; i < MAX_VALUE_SIZE; i++) {
                int global_idx = param_idx * MAX_VALUE_SIZE + i;
                int local_idx = local_param_idx * MAX_VALUE_SIZE + i;

                if (global_idx < PARAMETR_SIZE * MAX_VALUE_SIZE &&
                    local_idx < PARAMS_PER_BLOCK * MAX_VALUE_SIZE) {
                    global_pheromon[global_idx] = s_pheromon[local_idx];
                    global_kol_enter[global_idx] = s_kol_enter[local_idx];
                }
            }
        }
    }
...
PerformanceMetrics run_unified_aco_iterations(int num_iterations, bool use_dummy_first_iteration = true) {
    ...
    // УПРОЩЕННАЯ И БЕЗОПАСНАЯ ЛОГИКА
    bool use_shared_memory = false;
    int threads_per_block = 0;
    int blocks_per_grid = 0;
    size_t shared_memory_required = 0;

    // ДЛЯ БОЛЬШИХ PARAMETR_SIZE ВСЕГДА ИСПОЛЬЗУЕМ GLOBAL MEMORY
    if (PARAMETR_SIZE > 1) {
        threads_per_block = std::min(BLOCK_SIZE, PARAMETR_SIZE);
        blocks_per_grid = (PARAMETR_SIZE + threads_per_block - 1) / threads_per_block;
        shared_memory_required = 0;
        use_shared_memory = false;
        //std::cout << "Large PARAMETR_SIZE=" << PARAMETR_SIZE << ", using Global Memory strategy" << std::endl;
    }
    else {
        // Для маленьких размеров используем shared memory

            for (int max_safe_threads : {8, 16, 32, 64, 128, 256}) {
                if (max_safe_threads > PARAMETR_SIZE) break;

                int blocks = (PARAMETR_SIZE + max_safe_threads - 1) / max_safe_threads;
                int params_per_block = std::min(max_safe_threads, PARAMETR_SIZE - (blocks - 1) * max_safe_threads);
                size_t total_memory = params_per_block * MAX_VALUE_SIZE * 2 * sizeof(double) + PARAMETR_SIZE * (sizeof(double) + sizeof(int)) + sizeof(double) + sizeof(curandState);;
                    threads_per_block = max_safe_threads;
                    blocks_per_grid = blocks;
                    shared_memory_required = total_memory;
                    use_shared_memory = true;

            }
        }
 
    // Вычисляем параметров на блок для шаблона
    int params_per_block = std::min(threads_per_block, PARAMETR_SIZE - (blocks_per_grid - 1) * threads_per_block);

    CUDA_CHECK(cudaEventRecord(start, compute_stream));
    if (use_shared_memory) {
        // Используем shared memory версию
        ComputePheromonesAdaptive<true, 32> << <blocks_per_grid, threads_per_block, shared_memory_required, compute_stream >> > (
            dev_pheromon, dev_kol_enter, dev_norm_matrix, dev_agent_node, dev_OF,
#if (MAX_VALUE_SIZE * PARAMETR_SIZE > MAX_CONST)
            dev_parametr_value,
#endif
#if (GO_HASH)
            dev_hashTable,
#endif
            dev_max, dev_min, dev_hash_fail, num_iterations);
    }
    else {
        // Используем global memory версию  
        std::cout << "blocks_per_grid=" << blocks_per_grid<< " threads_per_block=" << threads_per_block << std::endl;
        ComputePheromonesAdaptive<false, 1> << <blocks_per_grid, threads_per_block, 0, compute_stream >> > (
            dev_pheromon, dev_kol_enter, dev_norm_matrix, dev_agent_node, dev_OF,
#if (MAX_VALUE_SIZE * PARAMETR_SIZE > MAX_CONST)
            dev_parametr_value,
#endif
    ...
    // Проверяем наличие ошибок после выполнения ядра
    cudaError_t kernelError = cudaGetLastError();
    if (kernelError != cudaSuccess) {
        std::cerr << "Unified kernel error: " << cudaGetErrorString(kernelError) << std::endl;
        return metrics;
    }
    ...
    metrics.memory_time_ms = total_time_ms - kernel_time;

    // Разделение времени на фазы (приблизительно)
    metrics.computeProbabilities_time_ms = kernel_time * 0.2f;
    metrics.antColonyOptimization_time_ms = kernel_time * 0.5f;
    metrics.updatePheromones_time_ms = kernel_time * 0.3f;

    // Расчет пропускной способности памяти
    size_t memory_transfers_per_iteration =
        (MAX_VALUE_SIZE * PARAMETR_SIZE * sizeof(double)) * 2 + // pheromon, kol_enter
        (PARAMETR_SIZE * ANT_SIZE * sizeof(int)) +              // agent_node
        (ANT_SIZE * sizeof(double));                            // OF

    size_t total_data_transferred = memory_transfers_per_iteration * num_iterations;

#if (MAX_VALUE_SIZE * PARAMETR_SIZE > MAX_CONST)
    total_data_transferred += (MAX_VALUE_SIZE * PARAMETR_SIZE * sizeof(double)) * num_iterations;
#endif

    metrics.memory_throughput_gbs = (total_data_transferred / (1024.0 * 1024.0 * 1024.0)) / (total_time_ms / 1000.0);

    // Расчет occupancy (используем реальные значения из конфигурации)
    ...
    int shared_mem_per_block = shared_memory_required;
    int shared_mem_per_sm = prop.sharedMemPerMultiprocessor;
    int max_blocks_by_shared_mem = (shared_mem_per_block > 0) ? shared_mem_per_sm / shared_mem_per_block :
        prop.maxBlocksPerMultiProcessor;

    int max_blocks_by_threads = max_threads_per_sm / threads_per_block;
    int actual_max_blocks = std::min(max_blocks_by_shared_mem, max_blocks_by_threads);

    int theoretical_warps = warps_per_block * actual_max_blocks;
    metrics.occupancy = std::min(100.0f, (theoretical_warps * 1.0f / max_warps_per_sm) * 100.0f);
...
PerformanceMetrics run_aco_iterations_4function(int num_iterations) {
    PerformanceMetrics metrics = { 0 };
    auto total_start = std::chrono::high_resolution_clock::now();
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
...
#if (MAX_VALUE_SIZE * PARAMETR_SIZE < MAX_CONST)
        antColonyOptimization << <ant_blocks, threads_per_block, sharedev_mem, compute_stream >> > (dev_norm_matrix, dev_agent_node, dev_OF,
#if (GO_HASH)
            dev_hashTable,
#endif
            dev_max, dev_min, dev_hash_fail); // 2. Оптимизация муравьями
#elif (PARAMETR_SIZE < MAX_SHARED)
        antColonyOptimization << <ant_blocks, threads_per_block, sharedev_mem, compute_stream >> > (dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF,
#if (GO_HASH)
            dev_hashTable,
#endif
            dev_max, dev_min, dev_hash_fail); // 2. Оптимизация муравьями
#else
        antColonyOptimization << <ant_blocks, threads_per_block, 0, compute_stream >> > (dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF,
#if (GO_HASH)
            dev_hashTable,
#endif
        ...
        // Фаза 3: Испарение феромонов (оптимизированная версия)
        evaporatePheromones << <(PARAMETR_SIZE + (BLOCK_SIZE - 1)) / BLOCK_SIZE, BLOCK_SIZE, 0, compute_stream >> > (dev_pheromon);

        // Фаза 4: Добавление феромонов (оптимизированная версия)
        depositPheromones << <(PARAMETR_SIZE + (BLOCK_SIZE - 1)) / BLOCK_SIZE, BLOCK_SIZE, 0, compute_stream >> > (dev_OF, dev_agent_node, dev_pheromon, dev_kol_enter);
...
#if (MAX_VALUE_SIZE * PARAMETR_SIZE < MAX_CONST)
        antColonyOptimization << <ant_blocks, threads_per_block, PARAMETR_SIZE * sizeof(double), compute_stream >> > (dev_norm_matrix, dev_agent_node, dev_OF, 
#if (GO_HASH)
            dev_hashTable, 
#endif
            dev_max, dev_min, dev_hash_fail);
#elif (PARAMETR_SIZE < MAX_SHARED)
        antColonyOptimization << <ant_blocks, threads_per_block, PARAMETR_SIZE * sizeof(double), compute_stream >> > (dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF, 
#if (GO_HASH)
            dev_hashTable,
#endif
            dev_max, dev_min, dev_hash_fail);
...
int main() {
    ...
    for (int i = 0; i < KOL_PROGON_STATISTICS; ++i) {
        auto metrics = run_aco_iterations_4function(KOL_ITERATION);
    ...
    std::cout << "\n=== Starting unified ACO runs ===" << std::endl;
    total_start = std::chrono::high_resolution_clock::now();

    for (int i = 0; i < KOL_PROGON_STATISTICS; ++i) {
        auto metrics = run_unified_aco_iterations(KOL_ITERATION, true);
        print_metrics(metrics, "Unified Run 1start", i);
    }

    total_end = std::chrono::high_resolution_clock::now();
    total_duration = std::chrono::duration_cast<std::chrono::milliseconds>(total_end - total_start);
    std::cout << "Unified ACO total execution time: " << total_duration.count() << " ms" << std::endl;
  */
    // Очистка ресурсов
    cleanup_guard();
    logFile.close();
