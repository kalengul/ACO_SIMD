...
__global__ void updatePheromones(const bool go_transposed, const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon, double* __restrict__ kol_enter) {
...
__global__ void depositPheromones(const bool go_transposed, const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon, double* __restrict__ kol_enter) {
    if (go_transposed) {
        depositPheromones_dev_transposed(OF, agent_node, pheromon, kol_enter);
    }
    else {
...
PerformanceMetrics run_aco_iterations_4function(const bool go_transposed, int num_iterations) {
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        depositPheromones << <(PARAMETR_SIZE + (BLOCK_SIZE - 1)) / BLOCK_SIZE, BLOCK_SIZE, 0, compute_stream >> > (go_transposed, dev_OF, dev_agent_node, dev_pheromon, dev_kol_enter);
...
PerformanceMetrics run_aco_iterations(const bool go_transposed, int num_iterations) {
    PerformanceMetrics metrics = { 0 };
    auto total_start = std::chrono::high_resolution_clock::now();
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        */
        ...
        updatePheromones << <(PARAMETR_SIZE + (BLOCK_SIZE-1)) / BLOCK_SIZE, BLOCK_SIZE, 0, compute_stream >> > (go_transposed, dev_OF, dev_agent_node, dev_pheromon, dev_kol_enter); // 3. Обновление феромонов
...
PerformanceMetrics run_combined_iterations(const bool go_transposed, int num_iterations, bool use_dummy_first_iteration = true) {
    PerformanceMetrics metrics = { 0 };
    auto total_start = std::chrono::high_resolution_clock::now();
...
int main() {
    ...
    for (int i = 0; i < KOL_PROGREV; ++i) {
        ...
        run_aco_iterations(false, KOL_ITERATION);
    }
    ...
    for (int i = 0; i < KOL_PROGON_STATISTICS; ++i) {
        auto metrics = run_aco_iterations_4function(false, KOL_ITERATION);
    ...
    for (int i = 0; i < KOL_PROGON_STATISTICS; ++i) {
        auto metrics = run_aco_iterations(false, KOL_ITERATION);
    ...
    for (int i = 0; i < KOL_PROGON_STATISTICS; ++i) {
        auto metrics = run_combined_iterations(false, KOL_ITERATION, true); 
...
if (!initialize_cuda_resources(params_transposed.data(), pheromones_transposed.data(), visits_transposed.data())) {
    ...
    for (int i = 0; i < KOL_PROGON_STATISTICS; ++i) {
        auto metrics = run_aco_iterations_4function(true, KOL_ITERATION);
