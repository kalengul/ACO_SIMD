...
set EXE_NAME=aco_cuda_global_while.exe
...
for %%S in (%PARAM_SIZES%) do (
    ...
    for %%R in (%REGISTER_COUNTS%) do (
        ...
        echo Compile command: nvcc -O3 -arch=sm_61 -std=c++17 -o %EXE_NAME% aco_cuda_global_while.cu -Xcompiler "/O2 /fp:fast /openmp /MT" -use_fast_math -lcudart -maxrregcount=%%R -Xptxas "-O3,-v" --resource-usage >> "!COMPILE_LOG!"
        echo. >> "!COMPILE_LOG!"
