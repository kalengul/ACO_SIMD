...
// Объявление константной памяти для параметров
__constant__ double dev_parametr_value_const[MAX_VALUE_SIZE * PARAMETR_SIZE];
...
__device__ void antColonyOptimization_dev(double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF, HashEntry* __restrict__ hashTable, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {
    ...
    for (int param_idx = tx; param_idx < PARAMETR_SIZE; param_idx += blockDim.x) {
        ...
        // Сохраняем выбранное значение параметра из константной памяти
        agent_params[param_idx] = dev_parametr_value_const[param_idx * MAX_VALUE_SIZE + selected_index];
    }
...
__device__ void antColonyOptimization_dev_transposed(double* __restrict__ norm_matrix_probability_transposed, int* __restrict__ agent_node, double* __restrict__ OF, HashEntry* __restrict__ hashTable, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {
    ...
    if (PARAMETR_SIZE <= 1024) {
        ...
        for (int i = 0; i < PARAMETR_SIZE; i++) {
            ...
            // Доступ к константной памяти
            agent_params[i] = dev_parametr_value_const[selected_index * PARAMETR_SIZE + i];
        }
...
__global__ void antColonyOptimization(const bool go_transposed, double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF, HashEntry* __restrict__ hashTable, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer) {  // Pre-allocated buffer
    if (go_transposed) {
        antColonyOptimization_dev_transposed(norm_matrix_probability, agent_node, OF, hashTable, maxOf_dev, minOf_dev, kol_hash_fail, global_params_buffer);
    }
    else {
        antColonyOptimization_dev(norm_matrix_probability, agent_node, OF, hashTable, maxOf_dev, minOf_dev, kol_hash_fail, global_params_buffer);
    }
}
...
// ==================== ПАРАЛЛЕЛЬНОЕ ЯДРО ====================
__global__ void parallelACOIteration(const double* __restrict__ OF, const int* __restrict__ agent_node, double* __restrict__ pheromon, double* __restrict__ kol_enter, double* __restrict__ norm_matrix_probability, double* __restrict__ norm_matrix_probability_ant, int* __restrict__ agent_node_ant, double* __restrict__ OF_ant, HashEntry* __restrict__ hashTable, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ dev_agent_params) {
...
#pragma unroll
        for (int param_idx = ant_tx; param_idx < PARAMETR_SIZE; param_idx += ant_threads) {
            ...
            // Доступ к константной памяти
            ant_params[param_idx] = dev_parametr_value_const[param_idx * MAX_VALUE_SIZE + low];
        }
#else
#pragma unroll
        for (int param_idx = ant_tx; param_idx < PARAMETR_SIZE; param_idx += ant_threads) {
            ...
            // Доступ к константной памяти
            ant_params[param_idx] = dev_parametr_value_const[param_idx * MAX_VALUE_SIZE + k];
        }
#endif
...
bool initialize_cuda_resources(const double* params, const double* pheromon, const double* kol_enter) {
    ...
    CUDA_CHECK(cudaMemcpyToSymbolAsync(dev_parametr_value_const, params, matrix_size, 0, cudaMemcpyHostToDevice, compute_stream));
...
PerformanceMetrics run_aco_iterations_4function(const bool go_min_parametrs, const bool go_transposed, int num_iterations) {
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        antColonyOptimization << <ant_blocks, threads_per_block, 0, compute_stream >> > (go_transposed, dev_norm_matrix, dev_agent_node, dev_OF, dev_hashTable, dev_max, dev_min, dev_hash_fail, dev_agent_params);
...
double calculate_gpu_memory_usage() {
    ...
    //    total_memory += HASH_TABLE_SIZE * sizeof(HashEntry);             // dev_hashTable
        ...
        // Константная память (уже не добавляется в общую сумму, т.к. это отдельный регион)
