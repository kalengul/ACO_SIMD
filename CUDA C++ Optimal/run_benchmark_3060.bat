...
:: Дополнительные флаги оптимизации для RTX 3060
set AMPERE_FLAGS=-DUSE_AMPERE_OPTIMIZATIONS=1 -DUSE_ASYNC_COPY=1 -DUSE_L2_PREFETCH=1 -DCUDA_ARCH=860
...
for %%S in (%PARAM_SIZES%) do (
    ...
    for %%R in (%REGISTER_COUNTS%) do (
        ...
        echo Compile command: nvcc -O3 -arch=sm_70 -std=c++17 -o %EXE_NAME% rtx3060_optimizations.cu -Xcompiler "/O2 /fp:fast /openmp /MT" -use_fast_math -lcudart -maxrregcount=%%R -Xptxas "-O3,-v" --resource-usage >> "!COMPILE_LOG!"
        echo. >> "!COMPILE_LOG!"
        
        nvcc -O3 -arch=!ARCH! -std=c++17 -o %EXE_NAME% rtx3060_optimizations.cu ^
