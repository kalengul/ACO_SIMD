...
set EXE_NAME=aco_cuda_global_4layers.exe
set OUTPUT_DIR=results
set CONFIG_FILE=parametrs4.h
...
for %%S in (%PARAM_SIZES%) do (
    ...
    :: Обновление параметра в файле parametrs4.h
    ...
    for %%R in (%REGISTER_COUNTS%) do (
        ...
        echo Compile command: nvcc -O3 -arch=sm_70 -std=c++17 -o %EXE_NAME% aco_cuda_global_4layers.cu -Xcompiler "/O2 /fp:fast /openmp /MT" -use_fast_math -lcudart -maxrregcount=%%R -Xptxas "-O3,-v" --resource-usage >> "!COMPILE_LOG!"
        echo. >> "!COMPILE_LOG!"
        
        nvcc -O3 -arch=!ARCH! -std=c++17 -o %EXE_NAME% aco_cuda_global_4layers.cu ^
...
set "graph_file=Parametr_Graph/test!new_size!_4.txt"

echo Updating parametrs4.h with PARAMETR_SIZE=!new_size! and NAME_FILE_GRAPH=!graph_file!
...
echo Successfully updated parametrs4.h with PARAMETR_SIZE=!new_size!
endlocal & exit /b 0
