...
#include <mpi.h>
int start_MPI() {
    MPI_Init(nullptr, nullptr); // Инициализация MPI
    int world_size;
    MPI_Comm_size(MPI_COMM_WORLD, &world_size); // Получение количества процессов
    int world_rank;
    ...
    double* kol_enter_value = new double[kolBytes_matrix_graph];
    double* norm_matrix_probability = new double[kolBytes_matrix_graph];
    double* ant = new double[PARAMETR_SIZE * ANT_SIZE]; // Учитываем распределение по процессам
    int* ant_parametr = new int[PARAMETR_SIZE * ANT_SIZE];
    double* antOF = new double[ANT_SIZE];

    // Загрузка матрицы из файла (только для процесса 0)
    if (world_rank == 0) {
        load_matrix_non_cuda(NAME_FILE_GRAPH, parametr_value, pheromon_value, kol_enter_value);
    }

    // Распространяем данные на все процессы
    ...
    for (int nom_iter = 0; nom_iter < KOL_ITERATION; ++nom_iter) {
        ...
        // Вычисление пути агентов
        //go_all_agent_non_cuda(int(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start).count() * CONST_RANDOM),parametr_value, norm_matrix_probability, ant + world_rank * ANT_SIZE, ant_parametr + world_rank * ANT_SIZE, antOF, hashTable);

        // Обновление весов-феромонов
        add_pheromon_iteration_non_cuda(pheromon_value, kol_enter_value, ant_parametr + world_rank * ANT_SIZE, antOF);

        // Поиск максимума и минимума локально
        double maxOf = -std::numeric_limits<double>::max();
        double minOf = std::numeric_limits<double>::max();
        for (int i = 0; i < ANT_SIZE; ++i) {
            if (antOF[i] != ZERO_HASH_RESULT) {
        ...
        // Обновление глобальных максимумов и минимумов
        MPI_Reduce(&minOf, &global_minOf, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
        MPI_Reduce(&maxOf, &global_maxOf, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    }
