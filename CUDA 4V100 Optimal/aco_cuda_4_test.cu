...
#undef ALLOCATE_AND_CHECK
    ...
    std::cout << "Process " << mpi_world_rank << ": CUDA resources initialized successfully" << std::endl;
    return true;
...
PerformanceMetrics run_combined_iterations_mpi(const bool go_min_parametrs, const bool go_transposed, int num_iterations) {
    ...
    if (g_resources.compute_stream == nullptr) {
        std::cerr << "Process " << mpi_world_rank << ": CUDA resources not initialized!" << std::endl;
        return metrics;
    }

    cuda_status = cudaEventCreate(&start);
    if (cuda_status != cudaSuccess) {
        std::cerr << "Process " << mpi_world_rank << ": Failed to create CUDA event (start). Error: "
            << cudaGetErrorString(cuda_status) << std::endl;
        return metrics;
    }
    ...
    if (cuda_status != cudaSuccess) {
        std::cerr << "Process " << mpi_world_rank << ": Failed to create CUDA event (start_ant). Error: "
            << cudaGetErrorString(cuda_status) << std::endl;
    ...
    if (cuda_status != cudaSuccess) {
        std::cerr << "Process " << mpi_world_rank << ": Failed to create CUDA event (stop). Error: "
            << cudaGetErrorString(cuda_status) << std::endl;
        cudaEventDestroy(start);
    ...
    if (mpi_is_master) {
        std::cout << "Process " << mpi_world_rank << ": Allocating global buffers for master" << std::endl;
        ...
        try {
            ...
            std::cout << "Process " << mpi_world_rank << ": Global buffers allocated and initialized" << std::endl;
        }
        catch (const std::bad_alloc& e) {
        ...
        std::cout << "Process " << mpi_world_rank << ": Allocating reduction buffers for slave" << std::endl;
        // Для не-мастер процессов выделяем временные буферы для редукции
        try {
            global_pheromon_reduction = new double[matrix_size];
            global_kol_enter_reduction = new double[matrix_size];
            std::cout << "Process " << mpi_world_rank << ": Reduction buffers allocated" << std::endl;
        }
        catch (const std::bad_alloc& e) {
    ...
    std::cout << "Process " << mpi_world_rank << ": Copying initial values to GPU" << std::endl;
    cuda_status = cudaMemcpyAsync(g_resources.dev_max, &max_init, sizeof(double),
        cudaMemcpyHostToDevice, g_resources.compute_stream);
    if (cuda_status != cudaSuccess) {
        std::cerr << "Process " << mpi_world_rank << ": Failed to copy max_init. Error: "
            << cudaGetErrorString(cuda_status) << std::endl;
        goto cleanup;
    }

    cuda_status = cudaMemcpyAsync(g_resources.dev_min, &min_init, sizeof(double),
        cudaMemcpyHostToDevice, g_resources.compute_stream);
    if (cuda_status != cudaSuccess) {
        std::cerr << "Process " << mpi_world_rank << ": Failed to copy min_init. Error: "
            << cudaGetErrorString(cuda_status) << std::endl;
        goto cleanup;
    }

#if (GO_HASH)
    cuda_status = cudaMemcpyAsync(g_resources.dev_hash_fail, &fail_init, sizeof(int),
        cudaMemcpyHostToDevice, g_resources.compute_stream);
    if (cuda_status != cudaSuccess) {
        std::cerr << "Process " << mpi_world_rank << ": Failed to copy hash_fail. Error: "
            << cudaGetErrorString(cuda_status) << std::endl;
        goto cleanup;
    }
#endif
    ...
    {
        std::cout << "Process " << mpi_world_rank << ": Initializing probability matrices" << std::endl;
        ...
        // Копируем на GPU
        cuda_status = cudaMemcpyAsync(g_resources.dev_norm_matrix, init_norm_matrix.data(),
            matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to copy initial norm matrix. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            goto cleanup;
        }
        ...
        if (mpi_is_master) {
            cuda_status = cudaMemcpyAsync(g_resources.dev_pheromon, global_pheromon,
                matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to copy initial pheromon. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                goto cleanup;
            }

            cuda_status = cudaMemcpyAsync(g_resources.dev_kol_enter, global_kol_enter,
                matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to copy initial kol_enter. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                goto cleanup;
            }
        }
            ...
            cuda_status = cudaMemcpyAsync(g_resources.dev_pheromon, zeros.data(),
                matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to initialize pheromon with zeros. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                goto cleanup;
            }

            cuda_status = cudaMemcpyAsync(g_resources.dev_kol_enter, zeros.data(),
                matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to initialize kol_enter with zeros. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                goto cleanup;
            }
        }
        ...
        cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed during initialization. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            goto cleanup;
        }
    }
    ...
    std::cout << "Process " << mpi_world_rank << ": Starting MPI synchronization" << std::endl;
    mpi_synchronize();
    std::cout << "Process " << mpi_world_rank << ": MPI synchronization 1 complete" << std::endl;

    mpi_broadcast_norm_matrix(g_resources.host_norm_matrix_buffer, matrix_size);
    std::cout << "Process " << mpi_world_rank << ": Norm matrix broadcast complete" << std::endl;

    mpi_synchronize();
    std::cout << "Process " << mpi_world_rank << ": MPI synchronization 2 complete" << std::endl;

    // Основной цикл итераций
    std::cout << "Process " << mpi_world_rank << ": Starting main loop with " << num_iterations << " iterations" << std::endl;
    for (int iter = 0; iter < num_iterations; ++iter) {
        if (mpi_is_master && (iter % 50 == 0)) {
            std::cout << "Process " << mpi_world_rank << ": Iteration " << iter << "/" << num_iterations << std::endl;
        }
        ...
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to record start_ant event. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }

        // Запуск оптимизации муравьев
        std::cout << "Process " << mpi_world_rank << ": Launching ant optimization kernel at iteration " << iter << std::endl;
        antColonyOptimizationAndDeposit << <ant_blocks, threads_per_block, 0, g_resources.compute_stream >> >
            (g_resources.dev_parametr_value, g_resources.dev_norm_matrix, g_resources.dev_hashTable,
                g_resources.dev_max, g_resources.dev_min, g_resources.dev_hash_fail,
                g_resources.dev_pheromon, g_resources.dev_kol_enter,
                g_resources.dev_agent_node, g_resources.dev_OF, g_resources.dev_agent_params);

        cuda_status = cudaGetLastError();
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Kernel launch failed at iteration "
                << iter << ". Error: " << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }

        cuda_status = cudaEventRecord(stop, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to record stop event. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed at iteration "
                << iter << ". Error: " << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to get elapsed time. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        std::cout << "Process " << mpi_world_rank << ": Copying data to host at iteration " << iter << std::endl;
        cuda_status = cudaMemcpyAsync(g_resources.host_pheromon_buffer, g_resources.dev_pheromon,
            matrix_bytes, cudaMemcpyDeviceToHost, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to copy pheromon to host. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }

        cuda_status = cudaMemcpyAsync(g_resources.host_kol_enter_buffer, g_resources.dev_kol_enter,
            matrix_bytes, cudaMemcpyDeviceToHost, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to copy kol_enter to host. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed after copy. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }

        // Собираем локальные феромоны и посещения на главном процессе через редукцию
        std::cout << "Process " << mpi_world_rank << ": Starting MPI reduce at iteration " << iter << std::endl;
        mpi_synchronize();
        ...
        std::cout << "Process " << mpi_world_rank << ": MPI reduce complete at iteration " << iter << std::endl;
        mpi_synchronize();

        // Главный процесс обновляет глобальные феромоны
        if (mpi_is_master) {
            std::cout << "Process " << mpi_world_rank << ": Master updating global pheromones at iteration " << iter << std::endl;
            cuda_status = cudaEventRecord(start, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to record start event. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }
            ...
            cuda_status = cudaMemcpyAsync(g_resources.dev_pheromon, global_pheromon,
                matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to copy global pheromon to GPU. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }
            ...
            cuda_status = cudaMemcpyAsync(g_resources.dev_kol_enter, global_kol_enter,
                matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to copy global kol_enter to GPU. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }
            ...
            cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed before kernel. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }
            ...
            std::cout << "Process " << mpi_world_rank << ": Launching computeProbabilities kernel at iteration " << iter << std::endl;
            computeProbabilitiesAndEvaporete << <blocks, BLOCK_SIZE, 0, g_resources.compute_stream >> >
                (go_transposed, g_resources.dev_pheromon, g_resources.dev_kol_enter, g_resources.dev_norm_matrix);

            cuda_status = cudaGetLastError();
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": ComputeProbabilities kernel failed at iteration "
                    << iter << ". Error: " << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }

            // Копируем для рассылки
            cuda_status = cudaMemcpyAsync(g_resources.host_norm_matrix_buffer, g_resources.dev_norm_matrix,
                matrix_bytes, cudaMemcpyDeviceToHost, g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Failed to copy norm matrix for broadcast. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }
            ...
            cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
            if (cuda_status != cudaSuccess) {
                std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed. Error: "
                    << cudaGetErrorString(cuda_status) << std::endl;
                break;
            }
            ...
            std::cout << "Process " << mpi_world_rank << ": Master update complete at iteration " << iter << std::endl;
        }

        // Все процессы копируют новую матрицу на свои GPU
        std::cout << "Process " << mpi_world_rank << ": Broadcasting norm matrix at iteration " << iter << std::endl;
        mpi_synchronize();
        mpi_broadcast_norm_matrix(g_resources.host_norm_matrix_buffer, matrix_size);
        mpi_synchronize();
        std::cout << "Process " << mpi_world_rank << ": Broadcast complete at iteration " << iter << std::endl;

        cuda_status = cudaMemcpyAsync(g_resources.dev_norm_matrix, g_resources.host_norm_matrix_buffer,
            matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to copy broadcasted norm matrix to GPU. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed after broadcast. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        cuda_status = cudaMemcpyAsync(g_resources.dev_pheromon, zeros.data(),
            matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to reset pheromon. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        cuda_status = cudaMemcpyAsync(g_resources.dev_kol_enter, zeros.data(),
            matrix_bytes, cudaMemcpyHostToDevice, g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Failed to reset kol_enter. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }
        ...
        cuda_status = cudaStreamSynchronize(g_resources.compute_stream);
        if (cuda_status != cudaSuccess) {
            std::cerr << "Process " << mpi_world_rank << ": Stream synchronization failed after reset. Error: "
                << cudaGetErrorString(cuda_status) << std::endl;
            break;
        }

        std::cout << "Process " << mpi_world_rank << ": Iteration " << iter << " complete" << std::endl;
        mpi_synchronize();
    }

cleanup:
    std::cout << "Process " << mpi_world_rank << ": Starting cleanup" << std::endl;
    mpi_synchronize();
    ...
    if (mpi_is_master) {
        ...
        std::cout << "Process " << mpi_world_rank << ": Master buffers freed" << std::endl;
    }
    else {
        delete[] global_pheromon_reduction;
        delete[] global_kol_enter_reduction;
        std::cout << "Process " << mpi_world_rank << ": Slave reduction buffers freed" << std::endl;
    }
    ...
    std::cout << "Process " << mpi_world_rank << ": Gathering statistics" << std::endl;
    cuda_status = cudaMemcpyAsync(&local_best_fitness, g_resources.dev_min, sizeof(double),
        cudaMemcpyDeviceToHost, g_resources.compute_stream);
    if (cuda_status == cudaSuccess) {
        cuda_status = cudaMemcpyAsync(&local_low_fitness, g_resources.dev_max, sizeof(double),
            cudaMemcpyDeviceToHost, g_resources.compute_stream);
    }

#if (GO_HASH)
    if (cuda_status == cudaSuccess) {
        cuda_status = cudaMemcpyAsync(&local_hash_fails, g_resources.dev_hash_fail, sizeof(int),
            cudaMemcpyDeviceToHost, g_resources.compute_stream);
    }
#endif
    ...
    if (mpi_is_master) {
        ...
        size_t total_data_transferred = (matrix_bytes * 4 * num_iterations * mpi_world_size) +
            (PARAMETR_SIZE * ANT_SIZE * sizeof(int)) * num_iterations * mpi_world_size +
            (ANT_SIZE * sizeof(double)) * num_iterations * mpi_world_size;
    ...
    std::cout << "Process " << mpi_world_rank << ": run_combined_iterations_mpi completed" << std::endl;
    return metrics;
}
