...
__device__ void antColonyOptimization_dev(double* __restrict__ dev_parametr_value, double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF, HashEntry* __restrict__ hashTable, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer, int ant_offset, int ant_count) {
     //СТАРАЯ НЕ ТРАНСПОНИРОВАННАЯ ВЕРСИЯ
    int bx = blockIdx.x;
    int tx = threadIdx.x;
    int ant_id = bx+ ant_offset;
...
__device__ void antColonyOptimization_dev_transposed(double* __restrict__ dev_parametr_value_transposed, double* __restrict__ norm_matrix_probability_transposed, int* __restrict__ agent_node, double* __restrict__ OF, HashEntry* __restrict__ hashTable, double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer, , int ant_offset, int ant_count) {
    // ВСЕГДА используем 1 поток = 1 агент для простоты и надежности
    int tid = blockIdx.x * blockDim.x + threadIdx.x+ant_offset;

    if (tid >= ant_count) return;
...
__global__ void antColonyOptimization(const bool go_transposed, double* __restrict__ dev_parametr_value, double* __restrict__ norm_matrix_probability, int* __restrict__ agent_node, double* __restrict__ OF,  HashEntry* __restrict__ hashTable,  double* __restrict__ maxOf_dev, double* __restrict__ minOf_dev, int* __restrict__ kol_hash_fail, double* __restrict__ global_params_buffer, int ant_offset, int ant_count) {  // Pre-allocated buffer
    if (go_transposed) {
        antColonyOptimization_dev_transposed(dev_parametr_value, norm_matrix_probability, agent_node, OF, hashTable, maxOf_dev, minOf_dev, kol_hash_fail, global_params_buffer, ant_offset, ant_count);
    }
    else {
        antColonyOptimization_dev(dev_parametr_value, norm_matrix_probability, agent_node, OF, hashTable, maxOf_dev, minOf_dev, kol_hash_fail, global_params_buffer, ant_offset, ant_count);
    }
}
...
// Буферы для обмена между GPU
double* dev_pheromon_send = nullptr;
double* dev_pheromon_recv = nullptr;
double* dev_kol_enter_send = nullptr;
double* dev_kol_enter_recv = nullptr;
...
bool initialize_mpi_and_cuda() {
    MPI_CHECK(MPI_Init(NULL, NULL));
    MPI_CHECK(MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank));
    MPI_CHECK(MPI_Comm_size(MPI_COMM_WORLD, &mpi_size));

    local_gpu_id = mpi_rank % 4;
    CUDA_CHECK(cudaSetDevice(local_gpu_id));

    std::cout << "MPI Rank " << mpi_rank << " using GPU " << local_gpu_id << std::endl;

    CUDA_CHECK(cudaStreamCreate(&compute_stream));
    return true;
}
bool allocate_resources(const double* params, const double* pheromon, const double* kol_enter) {
    const size_t matrix_size = MAX_VALUE_SIZE * PARAMETR_SIZE * sizeof(double);
    const size_t ant_matrix_size = PARAMETR_SIZE * ANT_SIZE * sizeof(int);
    ...
    CUDA_CHECK(cudaMallocAsync(&dev_pheromon_send, matrix_size, compute_stream));
    CUDA_CHECK(cudaMallocAsync(&dev_pheromon_recv, matrix_size, compute_stream));
    CUDA_CHECK(cudaMallocAsync(&dev_kol_enter_send, matrix_size, compute_stream));
    CUDA_CHECK(cudaMallocAsync(&dev_kol_enter_recv, matrix_size, compute_stream));
    ...
    extern __global__ void initializeHashTable(HashEntry*);
    initializeHashTable << <blocks, threads, 0, compute_stream >> > (dev_hashTable);
...
void cleanup_resources() {
    ...
    if (dev_pheromon) CUDA_CHECK(cudaFree(dev_pheromon));
    if (dev_kol_enter) CUDA_CHECK(cudaFree(dev_kol_enter));
    if (dev_norm_matrix) CUDA_CHECK(cudaFree(dev_norm_matrix));
    if (dev_agent_node) CUDA_CHECK(cudaFree(dev_agent_node));
    if (dev_OF) CUDA_CHECK(cudaFree(dev_OF));
    if (dev_max) CUDA_CHECK(cudaFree(dev_max));
    if (dev_min) CUDA_CHECK(cudaFree(dev_min));
    if (dev_hash_fail) CUDA_CHECK(cudaFree(dev_hash_fail));
    if (dev_hashTable) CUDA_CHECK(cudaFree(dev_hashTable));
    if (dev_parametr_value) CUDA_CHECK(cudaFree(dev_parametr_value));
    if (dev_agent_params) CUDA_CHECK(cudaFree(dev_agent_params));
    if (dev_pheromon_send) CUDA_CHECK(cudaFree(dev_pheromon_send));
    if (dev_pheromon_recv) CUDA_CHECK(cudaFree(dev_pheromon_recv));
    if (dev_kol_enter_send) CUDA_CHECK(cudaFree(dev_kol_enter_send));
    if (dev_kol_enter_recv) CUDA_CHECK(cudaFree(dev_kol_enter_recv));
...
PerformanceMetrics run_aco_iterations_4function(const bool go_min_parametrs, const bool go_transposed, int num_iterations) {
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        antColonyOptimization << <ant_blocks, threads_per_block, 0, compute_stream >> > (go_transposed,dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF, dev_hashTable, dev_max, dev_min, dev_hash_fail, dev_agent_params, 0, ANT_SIZE);
...
int get_local_ant_count() {
    return ANT_SIZE / mpi_size + (mpi_rank < ANT_SIZE % mpi_size ? 1 : 0);
}
int get_local_ant_offset() {
    int base = ANT_SIZE / mpi_size;
    int offset = base * mpi_rank;
    offset += std::min(mpi_rank, ANT_SIZE % mpi_size);
    return offset;
}
void synchronize_pheromones() {
    const size_t matrix_size = MAX_VALUE_SIZE * PARAMETR_SIZE * sizeof(double);

    CUDA_CHECK(cudaMemcpyAsync(dev_pheromon_send, dev_pheromon, matrix_size, cudaMemcpyDeviceToDevice, compute_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev_kol_enter_send, dev_kol_enter, matrix_size, cudaMemcpyDeviceToDevice, compute_stream));

    CUDA_CHECK(cudaStreamSynchronize(compute_stream));

    std::vector<double> pheromon_host(matrix_size / sizeof(double));
    std::vector<double> kol_enter_host(matrix_size / sizeof(double));

    CUDA_CHECK(cudaMemcpy(pheromon_host.data(), dev_pheromon_send, matrix_size, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(kol_enter_host.data(), dev_kol_enter_send, matrix_size, cudaMemcpyDeviceToHost));

    std::vector<double> pheromon_recv(matrix_size / sizeof(double));
    std::vector<double> kol_enter_recv(matrix_size / sizeof(double));

    MPI_CHECK(MPI_Allreduce(pheromon_host.data(), pheromon_recv.data(), MAX_VALUE_SIZE * PARAMETR_SIZE, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD));
    MPI_CHECK(MPI_Allreduce(kol_enter_host.data(), kol_enter_recv.data(), MAX_VALUE_SIZE * PARAMETR_SIZE, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD));

    CUDA_CHECK(cudaMemcpyAsync(dev_pheromon, pheromon_recv.data(), matrix_size, cudaMemcpyHostToDevice, compute_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev_kol_enter, kol_enter_recv.data(), matrix_size, cudaMemcpyHostToDevice, compute_stream));
}
void synchronize_best_values(double& local_min, double& local_max) {
    double global_min, global_max;
    MPI_CHECK(MPI_Allreduce(&local_min, &global_min, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD));
    MPI_CHECK(MPI_Allreduce(&local_max, &global_max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD));
    local_min = global_min;
    local_max = global_max;
    CUDA_CHECK(cudaMemcpyAsync(dev_min, &global_min, sizeof(double), cudaMemcpyHostToDevice, compute_stream));
    CUDA_CHECK(cudaMemcpyAsync(dev_max, &global_max, sizeof(double), cudaMemcpyHostToDevice, compute_stream));
}
PerformanceMetrics run_distributed_aco(const bool go_min_parametrs, const bool go_transposed, int num_iterations) {
    PerformanceMetrics metrics = { 0 };
    auto total_start = std::chrono::high_resolution_clock::now();
    ...
    float kernel_time = 0.0, computeProbabilities_time = 0.0;
    float antColonyOptimization_time = 0.0, updatePheromones_time = 0.0;

    int local_ant_count = get_local_ant_count();
    int local_ant_offset = get_local_ant_offset();

    int threads_per_block = std::min(PARAMETR_SIZE, BLOCK_SIZE);
    int ant_blocks = local_ant_count;
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        // Вычисление вероятностей
        extern __global__ void computeProbabilities(bool, const double*, const double*, double*);
        computeProbabilities << <(PARAMETR_SIZE + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, compute_stream >> > (go_transposed, dev_pheromon, dev_kol_enter, dev_norm_matrix);
        ...
        // Оптимизация муравьев
        antColonyOptimization << <ant_blocks, threads_per_block, 0, compute_stream >> > ( go_transposed, dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF, dev_hashTable, dev_max, dev_min, dev_hash_fail, dev_agent_params, local_ant_offset, local_ant_count);
      //antColonyOptimization << <ant_blocks, threads_per_block, 0, compute_stream >> > (go_transposed, dev_parametr_value, dev_norm_matrix, dev_agent_node, dev_OF, dev_hashTable, dev_max, dev_min, dev_hash_fail, dev_agent_params, 0, ANT_SIZE);
        ...
        // Добавление феромонов
        depositPheromones_distributed << <(PARAMETR_SIZE + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, compute_stream >> > ( go_min_parametrs, go_transposed, dev_OF, dev_agent_node, dev_pheromon, dev_kol_enter, local_ant_offset, local_ant_count);
        ...
        if (iter % 10 == 0 || iter == num_iterations - 1) {
            synchronize_pheromones();
        }
    }

    double local_best_fitness, local_low_fitness;
    int local_hash_fails;

    CUDA_CHECK(cudaMemcpyAsync(&local_best_fitness, dev_min, sizeof(double), cudaMemcpyDeviceToHost, compute_stream));
    CUDA_CHECK(cudaMemcpyAsync(&local_low_fitness, dev_max, sizeof(double), cudaMemcpyDeviceToHost, compute_stream));
#if (GO_HASH)
    CUDA_CHECK(cudaMemcpyAsync(&local_hash_fails, dev_hash_fail, sizeof(int), cudaMemcpyDeviceToHost, compute_stream));
#endif
    CUDA_CHECK(cudaStreamSynchronize(compute_stream));

    synchronize_best_values(local_best_fitness, local_low_fitness);
    ...
    if (mpi_rank == 0) {
        metrics.min_fitness = local_best_fitness;
        metrics.max_fitness = local_low_fitness;
        metrics.hash_misses = global_hash_fails;
        metrics.hash_hits = num_iterations * ANT_SIZE - global_hash_fails;
    }

    cudaDeviceProp prop;
    CUDA_CHECK(cudaGetDeviceProperties(&prop, local_gpu_id));
    ...
    CUDA_CHECK(cudaEventDestroy(start_ant));
    CUDA_CHECK(cudaEventDestroy(start_update));
    ...
    size_t total_data_transferred = (MAX_VALUE_SIZE * PARAMETR_SIZE * sizeof(double)) * 3 * num_iterations + (PARAMETR_SIZE * local_ant_count * sizeof(int)) * num_iterations + (local_ant_count * sizeof(double)) * num_iterations;
    metrics.memory_throughput_gbs = (total_data_transferred / (1024.0 * 1024.0 * 1024.0)) / (metrics.total_time_ms / 1000.0);
