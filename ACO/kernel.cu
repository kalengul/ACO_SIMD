...
__device__ void go_ant_path_random_values(int tx, int bx, double* random_values, int nom_iteration, double* parametr, double* norm_matrix_probability, double* agent, double* agent_node) {
    int k = 0;
    while (k < PARAMETR_SIZE && random_values[(bx * PARAMETR_SIZE + tx) * (nom_iteration + 1)] > norm_matrix_probability[MAX_VALUE_SIZE * tx + k]) { //
        k++;
    }
    __syncthreads();
    // Запись подматрицы блока в глобальную память
...
__global__ void go_all_agent_only(int* gpuTime, double* parametr, double* norm_matrix_probability, double* random_values, double* agent, double* agent_node, double* OF, HashEntry* hashTable, double* maxOf_dev, double* minOf_dev, int* kol_hash_fail) {
    int bx = threadIdx.x + blockIdx.x * blockDim.x;  // индекс  (агента) 
    if  (bx < ANT_SIZE) {
        //int tx = threadIdx.x;  
        curandState state;
        curand_init(clock64()*bx+ gpuTime[0], bx, 0, &state); // Инициализация состояния генератора случайных чисел
...
__global__ void go_all_agent(int* gpuTime, double* parametr, double* norm_matrix_probability, double* random_values, double* agent, double* agent_node, double* OF, HashEntry* hashTable, double* maxOf_dev, double* minOf_dev , int* kol_hash_fail) {
    ...
    else
    {
        ...
        curand_init((bx * blockDim.x + tx)*clock64()+gpuTime[0], bx * blockDim.x + tx, 0, &state); // Инициализация состояния генератора случайных чисел
        go_ant_path(tx, bx, &state, parametr, norm_matrix_probability, agent, agent_node);
    }
    double cachedResult = getCachedResultOptimized(hashTable, agent_node, bx);
    int nom_iteration = 1;
...
__global__ void go_all_agent_non_hash(int* gpuTime, double* parametr, double* norm_matrix_probability, double* random_values, double* agent, double* agent_node, double* OF, double* maxOf_dev, double* minOf_dev, int* kol_hash_fail) {
    int bx = blockIdx.x;  // индекс  (столбца)
    int tx = threadIdx.x; // индекс  (агента) 
    unsigned long long seed = 123 + bx * ANT_SIZE + tx * PARAMETR_SIZE + gpuTime[0] + clock64();
    curandState state;
    if (CPU_RANDOM) {
        go_ant_path_random_values(tx, bx, random_values, 0, parametr, norm_matrix_probability, agent, agent_node);
    }
...
__global__ void go_all_agent_opt(double* pheromon, double* kol_enter, int* gpuTime, double* parametr, double* norm_matrix_probability, double* agent, double* agent_node, double* OF, HashEntry* hashTable, double* maxOf_dev, double* minOf_dev, int* kol_hash_fail) {
    ...
    curandState state;
    curand_init(clock64(), bx * blockDim.x + tx, 0, &state); // Инициализация состояния генератора случайных чисел
...
__global__ void go_all_agent_opt_only(double* pheromon, double* kol_enter, int* gpuTime, double* parametr, double* norm_matrix_probability, double* agent, double* agent_node, double* OF, HashEntry* hashTable, double* maxOf_dev, double* minOf_dev, int* kol_hash_fail) {
    ...
    //int tx = threadIdx.x; // индекс потока (столбца)
        ...
        curandState state;
        curand_init(clock64(), bx , 0, &state); // Инициализация состояния генератора случайных чисел
    for (int nom_iter = 0; nom_iter < KOL_ITERATION; ++nom_iter) {
        for (int tx = 0; tx < PARAMETR_SIZE; tx++) { // Проходим по всем параметрам
            //Нормализация слоя с феромоном
            go_mass_probability(tx, pheromon, kol_enter, norm_matrix_probability);
...
static int start_CUDA() {
    ...
    for (int nom_iter = 0; nom_iter < KOL_ITERATION; ++nom_iter) {
        ...
        go_all_agent << <kol_ant, kol_parametr >> > (gpuTime_dev, parametr_value_dev, norm_matrix_probability_dev, random_values_dev, antdev, ant_parametr_dev, antOFdev, hashTable_dev, maxOf_dev, minOf_dev, kol_hash_fail);
...
static int start_CUDA_Time() {
    auto start_temp = std::chrono::high_resolution_clock::now();
    // Создание обработчиков событий CUDA
    cudaEvent_t start, startAll, startAll1, start1, start2, start3, stop, start_hash, stop_hash;
    cudaEventCreate(&startAll);
    cudaEventRecord(startAll, 0);
...
static int start_CUDA_non_hash() {
    ...
    for (int nom_iter = 0; nom_iter < KOL_ITERATION; ++nom_iter) {
        ...
        cudaMemcpy(random_values_dev, random_values, numBytes_matrix_ant, cudaMemcpyHostToDevice);//Запись множества в память GPU  
        go_all_agent_non_hash << <kol_ant, kol_parametr >> > (gpuTime_dev, parametr_value_dev, norm_matrix_probability_dev, random_values_dev, antdev, ant_parametr_dev, antOFdev,maxOf_dev, minOf_dev, kol_hash_fail);
...
static int start_CUDA_ant() {
    ...
    for (int nom_iter = 0; nom_iter < KOL_ITERATION; ++nom_iter) {
        ...
        go_all_agent_only << <numBlocks, numThreads >> > (gpuTime_dev, parametr_value_dev, norm_matrix_probability_dev, random_values_dev, antdev, ant_parametr_dev, antOFdev, hashTable_dev, maxOf_dev, minOf_dev, kol_hash_fail);
...
#pragma omp parallel
    {
        std::default_random_engine generator(123 + gpuTime + omp_get_thread_num()); // Используем gpuTime и номер потока как начальное значение
        std::uniform_real_distribution<double> distribution(0.0, 1.0);
...
int start_omp() {
    ...
    for (int nom_iter = 0; nom_iter < KOL_ITERATION; ++nom_iter) {
        ...
        go_all_agent_omp(nom_iter, parametr_value, norm_matrix_probability, ant, ant_parametr, antOF, hashTable, kol_hash_fail, SumgpuTime4, SumgpuTime5);
