...
#define GO_HYBRID_OMP_NON_HASH 1
...
struct ACOData {
    ...
    double cpu_ants_statistics = 0.0, gpu_ants_statistics = 0.0, kol_iteration_сpu = 0.0, kol_iteration_gpu = 0.0; 
...
void cpu_thread_function_OMP(ACOData* aco) {
    ...
    for (int iteration = 0; iteration < KOL_ITERATION && !aco->stop_requested.load(); iteration++) {
        ...
        aco->kol_iteration_сpu++;
        aco->kol_iteration_gpu++;
    ...
    while (iteration < KOL_ITERATION*2 && !aco->stop_requested) {
        ...
        if (!aco->OMP_to_cpu_queue_current.empty()) {
            ...
            if (OMP_data.ants_count > 0) {
                ...
                aco->cpu_ants_statistics += OMP_data.ants_count;
                aco->kol_iteration_сpu ++;
            }
#if (PRINT_INFORMATION)
        ...
        if (!aco->gpu_to_cpu_queue_current.empty()) {
            ...
            if (gpu_data.ants_count > 0) {
                ...
                aco->gpu_ants_statistics += gpu_data.ants_count;
                aco->kol_iteration_gpu++;
            }
#if (PRINT_INFORMATION)
...
#endif  
        auto start_time_wait = std::chrono::high_resolution_clock::now();
        aco->cpu_ants_statistics += cpu_data.ants_count;
        aco->kol_iteration_сpu++;
...
int start_hybrid() {
...
#endif
    aco.cpu_ants_statistics = aco.cpu_ants_statistics / (ANT_SIZE * aco.kol_iteration_сpu);
    aco.gpu_ants_statistics = aco.gpu_ants_statistics / (ANT_SIZE * aco.kol_iteration_gpu);
    std::cout << "Time Hybrid OMP;" << duration << "; " << duration_iteration << "; "
        ...
        << aco.kol_iteration_сpu << "; " << aco.kol_iteration_gpu << "; "
