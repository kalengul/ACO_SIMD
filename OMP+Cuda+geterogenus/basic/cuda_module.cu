...
void cuda_run_iteration(const double* norm_matrix_probability, int* ant_parametr, double* antOF, double* global_minOf, double* global_maxOf, int* kol_hash_fail, int iteration, void (*completion_callback)(double*, int, int)) {
    if (!cuda_data.initialized) {
