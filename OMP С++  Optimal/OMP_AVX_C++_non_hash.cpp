...
void generate_agents(int iteration, double* __restrict params, double* __restrict probabilities, double* __restrict agents, int* __restrict paths, double* __restrict scores, PerformanceMetrics& metrics) noexcept {
    auto start = std::chrono::high_resolution_clock::now();
...
#pragma omp for schedule(static)
        for (int agent = 0; agent < num_agents; ++agent) {
            ...
            scores[agent] = benchmark_function(&agents[agent_base]);
        }
    }
...
PerformanceMetrics run_single_experiment(int num_threads, int iteration_count = KOL_ITERATION) {
    ...
    for (int iter = 0; iter < iteration_count; ++iter) {
        ...
        generate_agents(iter, params.data(), probabilities.data(), agents.data(), paths.data(), scores.data(), metrics);
